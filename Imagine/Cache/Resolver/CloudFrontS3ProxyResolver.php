...
namespace BrauneDigital\ImagineBundle\Imagine\Cache\Resolver;

use Liip\ImagineBundle\Binary\BinaryInterface;
use Liip\ImagineBundle\Imagine\Cache\Resolver\ResolverInterface;


class CloudFrontS3ProxyResolver implements ResolverInterface
{
    /**
    ...
    protected function rewriteUrl($url)
    {
        ...
        $url = str_replace($host, $proxyHost, $url);

        if ($this->resolver instanceof AwsS3Resolver) {
            $url = str_replace($this->resolver->getBucket() . '/', '', $url);
        }
